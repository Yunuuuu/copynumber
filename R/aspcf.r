...
#' and above which BAF probes are considered germline homozygous. Must be in
#' the range 0 to 1, default is 0.1 and 0.9 for the lower and upper limit,
#' respectively.
#' @param skew a numeric value used to determine whether there is allelic
#' skewness (one or two bands) in BAF. Default is 3. The larger the value the
...
#' Default is 4.
#' @param return.est logical value indicating whether a data frame holding LogR
#' estimates should be returned along with the segments. Default is FALSE,
#' which means that only segments are returned.
...
#' probe equals the mean of the segment where the probe is located.}
#' \item{segments}{a data frame describing each segment found. Each row
