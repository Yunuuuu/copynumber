...
#' copy number column(s) should give the sample IDss.
#' @param BAF a data frame on the same format and size as \code{logR}, with
#' chromosomes and local probe positions in the two first columns, and numeric
...
#' segmentation results found by \code{\link{aspcf}}.
...
#' appearance) in \code{logR}, or in \code{segments} if \code{logR} is
#' unspecified.
...
#' @param winsoutliers an optional data frame of the same size as \code{logR}
...
#' will represent the probe indices. Default is "pos".
  ...
  for (i in 1:nSample) {
    ...
    for (c in 1:nChrom) {
      ...
      if (!is.null(logR)) {
        ...
        # plot logR
        plotObs(
          y = logR[ind.chrom, ind.sample + 2], pos = logR[ind.chrom, 2], unit = pos.unit, winsoutliers = winsoutliers[ind.chrom, ind.sample + 2], type = "aspcf", xaxis = xaxis,
      ...
      if (!is.null(segments)) {
        ...
        for (s in 1:nSeg) {
          ...
          plotSegments(use.segments[use.segments[, 2] == k, , drop = FALSE],
            type = "aspcf", k = k, xaxis = xaxis, add = add, plot.ideo = TRUE, col = arg$seg.col[s],
      ...
      if (!is.null(BAF)) {
        ...
        plotObs(BAF[ind.chrom, ind.sample],
          pos = logR[ind.chrom, 2], unit = pos.unit, winsoutliers = NULL, type = "aspcf", xaxis = xaxis, k = k,
      ...
      if (!is.null(segments)) {
        ...
        for (s in 1:nSeg) {
          ...
          plotSegments(use.segments[use.segments[, 2] == k, , drop = FALSE],
            type = "aspcf", k = k, xaxis = xaxis, add = TRUE, plot.ideo = TRUE, col = arg$seg.col[s],
            lty = arg$seg.lty[s], lwd = arg$seg.lwd[s], frame = baf.frame, new = TRUE, unit = pos.unit, seg.lim = seg.lim2, data.lim = data.lim2, op = arg2
          )
